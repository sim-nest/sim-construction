//! Source file-size gate for the sim-construction workspace.
...
mod tests {
    ...
    fn entrypoints_have_smaller_hard_limits() {
        assert_eq!(
            limits_for(Path::new("crates/example/src/lib.rs")).hard,
            ENTRYPOINT_HARD_LIMIT
        );
        assert_eq!(
            limits_for(Path::new("crates/example/src/main.rs")).hard,
            ENTRYPOINT_HARD_LIMIT
        );
        assert_eq!(
            limits_for(Path::new("crates/example/src/mod.rs")).hard,
            ENTRYPOINT_HARD_LIMIT
        );
    }
    ...
    fn implementation_files_use_general_hard_limit() {
        assert_eq!(
            limits_for(Path::new("crates/example/src/parser.rs")).hard,
            GENERAL_HARD_LIMIT
        );
    }
}
