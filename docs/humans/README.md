...
| `feature/sim-construction/change-commercial-exposure` | `crate/sim-lib-construction-project` | 2 | Carry one stable construction ChangeId from instruction or notice through scope, time, supplier exposure, customer recovery, quotation, authority, forecast, execution, settlement, and closure while deriving exact non-duplicated commercial exposure views with ledger Amount. |
